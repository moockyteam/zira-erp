-- 1. Function to Register a Global Payment (FIFO Allocation)
CREATE OR REPLACE FUNCTION register_global_payment(
    p_customer_id UUID,
    p_amount DECIMAL,
...
BEGIN
    ...
    LOOP
        ...
        IF r_doc.type = 'INVOICE' THEN
            INSERT INTO invoice_payments (
                ...
                p_date,
                p_payment_method,
                CASE WHEN p_notes IS NOT NULL AND p_notes != '' THEN p_notes || ' (Alloc. Auto)' ELSE 'Paiement Global (Alloc. Auto)' END
    ...
    -- Recalculate customer balance triggers will fire automatically on insert
...
GRANT EXECUTE ON FUNCTION register_global_payment(UUID, DECIMAL, TEXT, TEXT, TIMESTAMP) TO authenticated;
GRANT EXECUTE ON FUNCTION register_global_payment(UUID, DECIMAL, TEXT, TEXT, TIMESTAMP) TO service_role;

-- 3. Force Schema Cache Reload (Critical for 'function not found' errors)
