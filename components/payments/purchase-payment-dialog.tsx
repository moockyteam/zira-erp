...
export function SupplierPaymentDialog({ isOpen, onOpenChange, supplier, companyId, onSuccess }: any) {
  ...
  const handleSave = async () => {
    ...
    else { toast.success("Paiement enregistré."); onSuccess(); onOpenChange(false); }
    setIsSaving(false);
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Paiement pour {supplier?.name}</DialogTitle></DialogHeader>
        <div className="space-y-4 py-4">
          <div><Label>Montant Payé</Label><Input type="number" value={amount} onChange={e => setAmount(e.target.value)} /></div>
          ...
          <div><Label>Notes / Références Commandes</Label><Input value={notes} onChange={e => setNotes(e.target.value)} /></div>
        </div>
        <DialogFooter>
