...
import { Printer, MoreVertical, Send, CheckCircle, XCircle, FilePlus2, Truck, Edit, Globe } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function QuoteActions({ quoteId, currentStatus, onStatusChange }: QuoteActionsProps) {
  ...
  const handlePrintWithLanguage = (language: string) => {
    window.open(`/dashboard/quotes/print/${quoteId}?print=true&lang=${language}`, "_blank")
  }
      ...
      <DropdownMenuContent>
        ...
        {LANGUAGES.map((lang) => (
          <DropdownMenuItem key={lang.code} onClick={() => handlePrintWithLanguage(lang.code)}>
            <Globe className="h-4 w-4 mr-2" /> {lang.flag} {lang.label}
          </DropdownMenuItem>
        ))}
        ...
        {currentStatus === "ENVOYE" && (
          <>
            <DropdownMenuItem onClick={() => updateStatus("CONFIRME")}>
              <CheckCircle className="h-4 w-4 mr-2 text-green-600" /> Confirmer
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => updateStatus("REFUSE")}>
              <XCircle className="h-4 w-4 mr-2 text-red-600" /> Refuser
            </DropdownMenuItem>
          </>
        )}
