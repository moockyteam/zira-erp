        ...
        <div className="overflow-x-auto border rounded-md">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 border-2 border-gray-800 font-semibold">Réf.</th>
                <th className="p-2 border-2 border-gray-800 font-semibold w-[30%]">Désignation</th>
              ...
              {quote.quote_lines.map((line: QuoteLinePreview, index: number) => {
                ...
                return (
                  <tr key={line.id || index} className="hover:bg-gray-50">
                    <td className="p-2 border-2 border-gray-800">{line.items?.reference || "N/A"}</td>
                    <td className="p-2 border-2 border-gray-800">{line.description}</td>
                    <td className="p-2 border-2 border-gray-800 text-right whitespace-nowrap">{line.quantity}</td>
                    <td className="p-2 border-2 border-gray-800 text-right whitespace-nowrap">
                    ...
                    <td className="p-2 border-2 border-gray-800 text-right whitespace-nowrap font-medium">
                      {lineTotalHTAfterRemise.toFixed(3)}
                    </td>
                    <td className="p-2 border-2 border-gray-800 text-right whitespace-nowrap font-semibold">
          ...
          <div className="lg:col-span-1 bg-gray-50 p-4 rounded-md border">
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{(quote.total_ht + quote.total_remise).toFixed(3)} TND</span>
              </div>
              {quote.total_remise > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Remises sur lignes :</span>
                  <span className="font-mono">- {quote.total_remise.toFixed(3)} TND</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-muted-foreground">Total HT Net :</span>
                <span className="font-mono">{quote.total_ht.toFixed(3)} TND</span>
              </div>
              ...
              {quote.total_fodec > 0 && (
                <div className="flex justify-between text-blue-600">
                  <span className="font-medium">FODEC (1%) :</span>
                  <span className="font-mono">+ {quote.total_fodec.toFixed(3)} TND</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-muted-foreground">Total TVA :</span>
                <span className="font-mono">+ {quote.total_tva.toFixed(3)} TND</span>
              </div>
              {quote.has_stamp && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Timbre Fiscal :</span>
                  <span className="font-mono">1.000 TND</span>
                </div>
              )}
              <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                <span className="font-bold">Total TTC :</span>
                <span className="font-mono">{quote.total_ttc.toFixed(3)} TND</span>
              </div>
            </div>
          </div>
        </footer>
