// Placez ce code dans : components/invoices/invoice-preview.tsx

"use client"

import Image from 'next/image';
import writtenNumber from 'written-number';
import { cn } from '@/lib/utils'; // Assurez-vous que cette importation est présente si vous l'utilisez
      ...
      <div className="max-w-4xl mx-auto p-8 print:p-4" style={{ width: '210mm', minHeight: '297mm' }}>
        ...
        <header className="flex justify-between items-start pb-4 mb-6 border-b-2 border-black">
          <div className="flex items-center gap-4">
            {invoice.companies?.logo_url && (
              <div className="relative w-24 h-24 print:w-20 print:h-20 flex-shrink-0">
                <Image src={invoice.companies.logo_url} alt="Logo" fill className="object-contain" />
              </div>
            )}
          ...
          <div className="text-right text-xs">
            ...
            <p><span className="font-semibold">Numéro :</span> {invoice.invoice_number}</p>
            <p><span className="font-semibold">Date :</span> {new Date(invoice.invoice_date).toLocaleDateString('fr-FR')}</p>
            <p><span className="font-semibold">Échéance :</span> {new Date(invoice.due_date).toLocaleDateString('fr-FR')}</p>
          </div>
        </header>
          ...
          <div className="p-3 border rounded-md bg-gray-50 text-xs">
           ...
           {invoice.delivery_enabled && (
             <div className="p-3 border rounded-md bg-gray-50 text-xs">
                ...
                <p><span className="font-semibold">Chauffeur:</span> {invoice.driver_name || 'N/A'}</p>
                <p><span className="font-semibold">Véhicule:</span> {invoice.vehicle_registration || 'N/A'}</p>
            </div>
        )}
        </section>
              ...
              {invoice.invoice_lines.map((line: any, index: number) => {
                ...
                return (
                  <tr key={line.id || index}>
                    ...
                    <td className="p-2 border-t text-right font-mono align-top">{line.remise_percentage.toFixed(2)}%</td>
        ...
        <div className="pt-4" style={{ pageBreakInside: 'avoid' }}>
          <footer className="grid grid-cols-2 gap-4 items-start">
            <div className="p-3 border rounded-md bg-gray-50 text-xs">
                ...
                <table className="w-full">
                    ...
                    <tbody>
                    { [19, 13, 7, 0].map(rate => {
                        const base = invoice.invoice_lines.filter(l => l.tva_rate === rate).reduce((s, l) => s + (l.quantity * l.unit_price_ht * (1 - l.remise_percentage/100)), 0);
                        if (base === 0) return null;
                        const base_fodec = base + (isFodecApplicable ? (base * (1 - invoice.escompte_percentage / 100)) * FODEC_RATE : 0);
            ...
            <div className="p-3 border rounded-md font-mono text-xs">
              <div className="flex justify-between"><span className="font-sans text-muted-foreground">Total HT Net</span><span>{invoice.total_ht.toFixed(3)}</span></div>
              {invoice.total_escompte > 0 && <div className="flex justify-between"><span className="font-sans text-muted-foreground">Escompte</span><span>- {invoice.total_escompte.toFixed(3)}</span></div>}
              {isFodecApplicable && <div className="flex justify-between"><span className="font-sans text-muted-foreground">FODEC (1%)</span><span>+ {invoice.total_fodec.toFixed(3)}</span></div>}
              <div className="flex justify-between"><span className="font-sans text-muted-foreground">Total TVA</span><span>+ {invoice.total_tva.toFixed(3)}</span></div>
              {invoice.has_stamp && <div className="flex justify-between"><span className="font-sans text-muted-foreground">Timbre Fiscal</span><span>+ 1.000</span></div>}
          ...
          <div className="mt-4 pt-4 border-t text-[8pt] italic text-gray-600">
              {totalInWords(invoice.total_ttc)}
          </div>
        </div>
      </div>
    </div>
