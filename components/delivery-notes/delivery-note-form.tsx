...
type Customer = {
  id: string; name: string; address: string | null; street?: string; delegation?: string;
  governorate?: string; country?: string; matricule_fiscal?: string; email?: string; phone_number?: string;
}
...
export function DeliveryNoteForm({ initialData, initialDataSource, companies, customers: initialCustomers, items }: any) {
  ...
  const handleLineChange = useCallback((index: number, field: keyof DnLine, value: any) => {
  ...
  const handleSave = async () => {
    ...
    const dnPayload = {
      ...
      // On n'envoie les totaux que si le BL est valorisé
      total_ht: isValued ? lines.reduce((sum, l) => sum + l.quantity * l.unit_price_ht * (1 - (l.remise_percentage || 0) / 100), 0) : 0,
      // ... (vous pouvez ajouter les autres totaux ici si nécessaire pour la DB)
    }

    if (isNew) {
      const { data: numberData } = await supabase.rpc('get_next_delivery_note_number', { p_company_id: companyId })
      const { data: newDn, error } = await supabase.from("delivery_notes").insert({ ...dnPayload, delivery_note_number: numberData }).select("id").single()
  ...
  return (
    <div className="space-y-6">
              ...
              {lines.map((line, index) => {
                ...
                return (
                  <TableRow key={line.local_id}>
                    <TableCell className="align-top">
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            {items.filter((i: Item) => i.name.toLowerCase().includes(line.description.toLowerCase())).slice(0, 5).map((i: Item) => (
                    ...
                    <TableCell className="align-top">
                      <Input type="number" value={line.quantity} onChange={(e) => handleLineChange(index, "quantity", Number.parseFloat(e.target.value) || 0)} className="text-right" />
                    </TableCell>
                    {isValued && <TableCell className="align-top"><Input type="number" value={line.unit_price_ht} onChange={(e) => handleLineChange(index, "unit_price_ht", Number.parseFloat(e.target.value) || 0)} className="text-right" /></TableCell>}
                    {isValued && showRemise && <TableCell className="align-top"><Input type="number" value={line.remise_percentage} onChange={(e) => handleLineChange(index, "remise_percentage", Number.parseFloat(e.target.value) || 0)} className="text-right" /></TableCell>}
                    {isValued && <TableCell className="align-top"><Select value={String(line.tva_rate)} onValueChange={(v) => handleLineChange(index, "tva_rate", Number.parseInt(v))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{TVA_RATES.map(r => <SelectItem key={r} value={String(r)}>{r}%</SelectItem>)}</SelectContent></Select></TableCell>}
                    {isValued && <TableCell className="align-top text-right pt-3 font-mono text-muted-foreground">{unitPriceTTC.toFixed(3)}</TableCell>}
                    {isValued && <TableCell className="align-top text-right pt-3 font-mono">{lineTotalHT.toFixed(3)}</TableCell>}
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button onClick={handleSave} disabled={isLoading}><Save className="mr-2 h-4 w-4"/>Enregistrer le BL</Button>
      </div>
    </div>
  )
}
