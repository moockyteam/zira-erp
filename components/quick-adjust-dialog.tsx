...
export function QuickAdjustDialog({ item, isOpen, onOpenChange, onSuccess }: QuickAdjustDialogProps) {
  ...
  const handleAdjust = async () => {
    ...
    const nQty = Number.parseFloat(newQuantity)
    const pQty = item.quantity_on_hand
    const adjustment = nQty - pQty

    // 1. Mettre à jour la quantité de l'article
    const { error: updateError } = await supabase.from("items").update({ quantity_on_hand: nQty }).eq("id", item.id)
      ...
      <DialogContent>
        ...
        <div className="space-y-4 py-4">
          ...
          <div>
            ...
            <Input
              id="reason"
              placeholder="Ex: Correction d'inventaire, Perte..."
              value={reason}
