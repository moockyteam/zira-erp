...
export function ReceiptDialog({ purchaseOrder, isOpen, onOpenChange, onSuccess }: ReceiptDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: numberData } = await supabase.rpc("get_next_receipt_number", {
        p_company_id: purchaseOrder.company_id,
      })
      ...
      const linesToInsert = lines
        .filter((line) => Number.parseFloat(line.quantity_received) > 0)
        .map((line) => {
          // CORRECTION: On vérifie que item_id n'est pas null
          if (!line.item_id) {
            throw new Error(
              `L'article "${line.description}" n'est pas lié à un article du stock. Impossible de le réceptionner.`,
            )
          }
          return {
            ...
            item_id: line.item_id,
            description: line.description,
            quantity_received: Number.parseFloat(line.quantity_received),
          }
        })
                ...
                <TableRow key={line.description}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) => handleQuantityChange(line.description, e.target.value.replace(",", "."))}
