...
export function PoPaymentManager({ isOpen, onOpenChange, purchaseOrder, onSuccess }: any) {
  ...
  const totalPaidOrPending = payments.reduce((sum, p) => sum + Number.parseFloat(p.total_ttc), 0)
  ...
  const handleAddManual = async () => {
    setIsSaving(true)
    const total_ttc = Number.parseFloat(amount)
    const payload = {
      company_id: purchaseOrder.company_id,
      supplier_id: purchaseOrder.supplier_id,
      purchase_order_id: purchaseOrder.id,
      beneficiary:
        (await supabase.from("suppliers").select("name").eq("id", purchaseOrder.supplier_id).single()).data?.name ||
        "N/A",
  ...
  const handleGenerateAuto = async () => {
    const totalToGen = Number.parseFloat(genTotalAmount) || amountDue
    ...
    const lastPartAmount = Number.parseFloat((totalToGen - partAmount * (count - 1)).toFixed(3))
      ...
      <DialogContent className="sm:max-w-4xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4 p-3 bg-muted rounded-lg border">
            <div className="text-center">
              ...
              <p className="font-bold">{purchaseOrder?.total_ttc.toFixed(3)}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-muted-foreground uppercase">Planifié/Payé</p>
              <p className="font-bold text-emerald-600">{totalPaidOrPending.toFixed(3)}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-muted-foreground uppercase">Reste</p>
              <p className="font-bold text-destructive">{amountDue.toFixed(3)}</p>
            </div>
          </div>
        </DialogHeader>
        ...
        <Tabs defaultValue="list" className="mt-4">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="auto" className="text-indigo-600 font-bold">
              ...
              <TableBody>
                {payments.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <Badge variant={p.status === "EN_ATTENTE" ? "outline" : "success"}>{p.status}</Badge>
                    </TableCell>
                    <TableCell>
          ...
          <TabsContent value="manual" className="space-y-4 py-4 border p-4 rounded-lg">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="date"
                  value={dueDate || paymentDate}
                  onChange={(e) =>
                    paymentMethod === "Chèque" || paymentMethod === "Traite"
              ...
              <div className="space-y-2">
                ...
                <Input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="N° Chèque, etc." />
              </div>
            </div>
            <Button onClick={handleAddManual} disabled={isSaving} className="w-full">
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Montant Total à diviser</Label>
                <Input
                  type="text"
                  value={genTotalAmount}
                  onChange={(e) => setGenTotalAmount(e.target.value.replace(",", "."))}
                  placeholder={amountDue.toFixed(3)}
                />
              </div>
              <div className="space-y-2">
                <Label>Nombre d'échéances</Label>
                <Input type="text" value={genCount} onChange={(e) => setGenCount(e.target.value)} />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label>Intervalle entre échéances (jours)</Label>
                <Input type="text" value={genInterval} onChange={(e) => setGenInterval(e.target.value)} />
              </div>
            </div>
            <div className="bg-white p-3 rounded border text-sm text-indigo-700">
              💡 Cela va créer <strong>{genCount || 0}</strong> paiements de{" "}
              <strong>
                {genTotalAmount && genCount
                  ? (Number.parseFloat(genTotalAmount) / Number.parseInt(genCount)).toFixed(3)
                  : 0}{" "}
                TND
              </strong>{" "}
              en mode Chèque.
