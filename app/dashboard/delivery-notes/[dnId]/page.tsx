...
export default async function DnEditorPage({ 
  ...
  if (!companies || companies.length === 0) {
    return (
        <div className="p-8 text-center">
            ...
            <p className="mb-6">Vous devez d'abord créer une entreprise avant de pouvoir créer un bon de livraison.</p>
  ...
  const { data: customers } = await supabase.from('customers').select('id, name, address').eq('company_id', companyIdForData);
  const { data: items } = await supabase.from('items').select('id, name, reference, quantity_on_hand, sale_price').eq('company_id', companyIdForData);
